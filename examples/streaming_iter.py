...
#     system_prompt="你是天气助手：根据用户提供的地点和日期，给出天气预报（必要时调用工具）。",
# )

weather_agent = MyAgent(
...
async def weather_forecast(
    ...
    if forecast_date >= date.today():
        return await ctx.deps.get_forecast(
            location,
            forecast_date,
        )
    else:
        return await ctx.deps.get_historic_weather(
            location,
            forecast_date,
        )
                ...
                async with node.stream(run.ctx) as request_stream:
                    ...
                    async for event in request_stream:
                        if isinstance(event, PartStartEvent):
                            ...
                            output_messages.append(f"[Result] The model started producing a final result (tool_name={event.tool_name})")
